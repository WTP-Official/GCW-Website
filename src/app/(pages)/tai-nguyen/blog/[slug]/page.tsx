...
// Refresh hourly — matches the blog list's cadence.
...
export default async function TaiNguyenBlogPostPage({
  params,
}: {
  params: Promise<Params>;
}) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-24">
      <Link href="/tai-nguyen/blog" className="text-sm font-medium text-brand-600 hover:text-brand-700">
        ← Về trang Tin tức
      </Link>
      ...
      {(post.prevArticle || post.nextArticle) && (
        ...
        >
          {post.prevArticle ? (
            <Link
              href={`/tai-nguyen/blog/${post.prevArticle.slug}`}
          ...
          {post.nextArticle ? (
            <Link
              href={`/tai-nguyen/blog/${post.nextArticle.slug}`}
        ...
        <div className="mt-12 border-t border-surface-3 pt-8">
          ...
          <div className="mt-4 flex flex-col gap-3">
            {post.relatedArticles.map((related) => (
              <Link
                key={related.slug}
                href={`/tai-nguyen/blog/${related.slug}`}
                className="text-brand-600 hover:text-brand-700"
              >
                {related.title}
              </Link>
