...
// Articles come from the external public API (same content source as the
// /blog proxy — see src/app/api/blog/topics/route.ts), so the page
...
export default async function TaiNguyenBlogPage({
  searchParams,
}: {
        ...
        <>
            ...
            {posts.map((post) => (
              ...
              >
                ...
                <h2 className="mt-2 text-xl">
                  <Link
                    href={`/tai-nguyen/blog/${post.slug}`}
          ...
          {!isSearching && list && list.totalPages > 1 && (
            ...
            >
              {page > 1 ? (
                <Link
                  href={`/tai-nguyen/blog?page=${page - 1}${topic ? `&topic=${encodeURIComponent(topic)}` : ""}`}
